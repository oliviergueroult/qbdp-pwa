...
export default function ResetMotDePasse() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await axios.post(`${BASE_URL}/auth/reset-with-token`, { token, new_password: password });
      setDone(true);
    } catch (err) {
  ...
  if (done) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: '#f4f5f7' }}>
      <div style={{ background: 'white', borderRadius: 20, padding: 32, width: '100%', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 20, fontWeight: 700, color: '#1a1a2e' }}>Mot de passe modifié !</div>
        <p style={{ color: '#6b7280', marginTop: 8, marginBottom: 24 }}>Vous pouvez maintenant vous connecter.</p>
        <button onClick={() => window.location.href = '/'} style={{ background: '#1a56db', color: 'white', border: 'none', borderRadius: 10, padding: '12px 28px', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>
          Se connecter
        </button>
      </div>
    </div>
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: '#f4f5f7' }}>
      <div style={{ background: 'white', borderRadius: 20, padding: 32, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <img src={logo2} style={{ width: 80, height: 80, borderRadius: 20, marginBottom: 12, objectFit: 'contain' }} alt="QBDP" />
          <div style={{ fontSize: 20, fontWeight: 700, color: '#1a1a2e' }}>Nouveau mot de passe</div>
          <p style={{ color: '#6b7280', fontSize: 14, marginTop: 6 }}>Choisissez un nouveau mot de passe</p>
        </div>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Nouveau mot de passe</div>
          <input type="password" value={password} onChange={e => setPassword(e.target.value)}
            placeholder="Minimum 6 caractères"
            style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 10, padding: '12px 14px', fontSize: 15, outline: 'none', boxSizing: 'border-box' }} />
        </div>
